...
const toRp = (n: number | undefined | null): string => {
...
export const exportToPDF = (
  ...
  try {
    ...
    if (!items || items.length === 0) {
      console.warn('Export PDF: Tidak ada item RAB untuk diexport');
    }
    ...
    doc.text('sivilize-hub-pro.vercel.app', margin + 22, 30);
    ...
    grouped.forEach((group) => {
      // Periksa apakah ada cukup ruang di halaman saat ini
      if (y > 260) {
        doc.addPage();
        y = 20;
      }
      ...
      // Update y berdasarkan posisi terakhir tabel
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`SIVILIZE HUB PRO | sivilize-hub-pro.vercel.app | Halaman ${i} dari ${pageCount}`, pageW / 2, 290, { align: 'center' });
      doc.setDrawColor(220, 220, 220); doc.setLineWidth(0.3);
      doc.line(margin, 287, pageW - margin, 287);
    }
    
    const fileName = `RAB_${(project.name || 'Proyek').replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(fileName);
  } catch (error) {
    console.error('Error generating PDF:', error);
    alert('Gagal menghasilkan PDF. Silakan coba lagi atau hubungi dukungan jika masalah berlanjut.');
  }
};
